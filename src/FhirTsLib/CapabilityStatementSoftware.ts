...
import { REGEX_ID, REGEX_NAME, REGEX_VERSION, REGEX_RELEASEDATE } from '../FhirTsLibValidation/FhirRegex';
 ...
 *    file:/Users/admin/Documents/DevNetbeans/jsonSchemaToJava/target/classes/schema/fhir.schema.json#/definitions/CapabilityStatement_Software
 */
export class CapabilityStatementSoftware {
  ...
  * that should be caught.
  */
  public setReleaseDate(newValue: string) {
    if (typeof(newValue) === 'undefined') {
      if (typeof(this.releaseDate) !== 'undefined') {
         delete this.releaseDate;
      }
      return;
    }
    
    if (REGEX_RELEASEDATE.test(newValue)) {
      this.releaseDate = newValue
    } else {
      throw new Error("Invalid value pattern for releaseDate of " + newValue);
    }
  }
  
  public getReleaseDatePattern(): string {
    return REGEX_RELEASEDATE.source;
  }
  
  /**
  * Gets the current value of field releaseDate.
  * @returns the value  or undefined
  */
  ...
  /**
  * Gets the current value of field releaseDate if it exists,
  * but if it is 'undefined' it will return the value supplied
  * as the parameter.
  * @returns the value or the parameter value
  */
  public getReleaseDateOrElse(defaultValue: string): string {
      if (this.releaseDate == undefined || this.releaseDate == null ) { return defaultValue; }
      return this.releaseDate;
  }
