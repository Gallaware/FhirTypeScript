 ...
 *    file:/Users/admin/Documents/DevNetbeans/jsonSchemaToJava/target/classes/schema/fhir.schema.json#/definitions/ResearchDefinition
 */
export class ResearchDefinition {
  ...
  /**
   * Descriptive topics related to the content of the ResearchDefinition. Topics provide a high-level
   * categorization grouping types of ResearchDefinitions that can be useful for filtering and
   * searching.
   * Field access restricted because there are specific valid values.
   */
  @Type(() => CodeableConcept)
  ...
  */
  public setStatus(newValue: string) {
    ...
    const idx = getValidValueIdx(ResearchDefinitionStatus_list_ValidValues, newValue);
    if (idx >= 0) {
      this.status = ResearchDefinitionStatus_list_ValidValues[idx];
    } else {
      throw new Error("Unknown status of " + newValue);
    }
  }
  
  public getResearchDefinitionStatusValidValues(): string[] {
    return ResearchDefinitionStatus_list_ValidValues;
  }
