...
import { REGEX_ID, REGEX_EXAMPLECANONICAL, REGEX_RELATIVEPATH } from '../FhirTsLibValidation/FhirRegex';
 ...
 *    file:/Users/admin/Documents/DevNetbeans/jsonSchemaToJava/target/classes/schema/fhir.schema.json#/definitions/ImplementationGuide_Resource1
 */
export class ImplementationGuideResource1 {
  /**
  ...
  * that should be caught.
  */
  public setRelativePath(newValue: string) {
    if (typeof(newValue) === 'undefined') {
      if (typeof(this.relativePath) !== 'undefined') {
         delete this.relativePath;
      }
      return;
    }
    
    if (REGEX_RELATIVEPATH.test(newValue)) {
      this.relativePath = newValue
    } else {
      throw new Error("Invalid value pattern for relativePath of " + newValue);
    }
  }
  
  public getRelativePathPattern(): string {
    return REGEX_RELATIVEPATH.source;
  }
  
  /**
  * Gets the current value of field relativePath.
  * @returns the value  or undefined
  */
