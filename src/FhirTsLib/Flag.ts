 ...
 *    file:/Users/admin/Documents/DevNetbeans/jsonSchemaToJava/target/classes/schema/fhir.schema.json#/definitions/Flag
 */
export class Flag {
  /**
  ...
  /**
   * Allows a flag to be divided into different categories like clinical, administrative etc. Intended
   * to be used as a means of filtering which flags are displayed to particular user or in a given
   * context.
   * Field access restricted because there are specific valid values.
   */
  ...
  */
  public setStatus(newValue: string) {
    ...
    const idx = getValidValueIdx(FlagStatus_list_ValidValues, newValue);
    if (idx >= 0) {
      this.status = FlagStatus_list_ValidValues[idx];
    } else {
      throw new Error("Unknown status of " + newValue);
    }
  }
  
  public getFlagStatusValidValues(): string[] {
    return FlagStatus_list_ValidValues;
  }
