 ...
 *    file:/Users/admin/Documents/DevNetbeans/jsonSchemaToJava/target/classes/schema/fhir.schema.json#/definitions/Appointment_Participant
 */
export class AppointmentParticipant {
  ...
  /**
   * Whether this participant is required to be present at the meeting. This covers a use-case where two
   * doctors need to meet to discuss the results for a specific patient, and the patient is not required
   * to be present.
   * Field access restricted because there are specific valid values.
   */
  ...
  * that should be caught.
  */
  public setRequired(newValue: string) {
    if (typeof(newValue) === 'undefined') {
    ...
    const idx = getValidValueIdx(AppointmentParticipantRequired_list_ValidValues, newValue);
    if (idx >= 0) {
      this.required = AppointmentParticipantRequired_list_ValidValues[idx];
    } else {
      throw new Error("Unknown required of " + newValue);
    }
  }
  
  public getAppointmentParticipantRequiredValidValues(): string[] {
    return AppointmentParticipantRequired_list_ValidValues;
  }
  
  /**
  * Gets the current value of field required.
  * @returns the value  or undefined
  */
  ...
  /**
  * Gets the current value of field required if it exists,
  * but if it is 'undefined' it will return the value supplied
  * as the parameter.
  * @returns the value or the parameter value
  */
  public getRequiredOrElse(defaultValue: string): string {
      if (this.required == undefined || this.required == null ) { return defaultValue; }
      return this.required;
  }
  ...
  */
  public setStatus(newValue: string) {
    ...
    const idx = getValidValueIdx(AppointmentParticipantStatus_list_ValidValues, newValue);
    if (idx >= 0) {
      this.status = AppointmentParticipantStatus_list_ValidValues[idx];
    } else {
      throw new Error("Unknown status of " + newValue);
    }
  }
  
  public getAppointmentParticipantStatusValidValues(): string[] {
    return AppointmentParticipantStatus_list_ValidValues;
  }
