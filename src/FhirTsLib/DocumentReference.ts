...
import { REGEX_ID_000, REGEX_IMPLICITRULES, REGEX_LANGUAGE, REGEX_DOCSTATUS, REGEX_DATE_001, REGEX_DESCRIPTION } from '../FhirTsLibValidation/FhirRegex';
import { CodeableConcept, DocumentReferenceContent, DocumentReferenceContext, DocumentReferenceRelatesTo, Element, Extension, Identifier, Meta, Narrative, Reference, ResourceList, Account, ActivityDefinition, AdverseEvent, AllergyIntolerance, Appointment, AppointmentResponse, AuditEvent, Basic, Binary, BiologicallyDerivedProduct, BodyStructure, Bundle, CapabilityStatement, CarePlan, CareTeam, CatalogEntry, ChargeItem, ChargeItemDefinition, Claim, ClaimResponse, ClinicalImpression, CodeSystem, Communication, CommunicationRequest, CompartmentDefinition, Composition, ConceptMap, Condition, Consent, Contract, Coverage, CoverageEligibilityRequest, CoverageEligibilityResponse, DetectedIssue, Device, DeviceDefinition, DeviceMetric, DeviceRequest, DeviceUseStatement, DiagnosticReport, DocumentManifest, EffectEvidenceSynthesis, Encounter, Endpoint, EnrollmentRequest, EnrollmentResponse, EpisodeOfCare, EventDefinition, Evidence, EvidenceVariable, ExampleScenario, ExplanationOfBenefit, FamilyMemberHistory, Flag, Goal, GraphDefinition, Group, GuidanceResponse, HealthcareService, ImagingStudy, Immunization, ImmunizationEvaluation, ImmunizationRecommendation, ImplementationGuide, InsurancePlan, Invoice, Library, Linkage, List, Location, Measure, MeasureReport, Media, Medication, MedicationAdministration, MedicationDispense, MedicationKnowledge, MedicationRequest, MedicationStatement, MedicinalProduct, MedicinalProductAuthorization, MedicinalProductContraindication, MedicinalProductIndication, MedicinalProductIngredient, MedicinalProductInteraction, MedicinalProductManufactured, MedicinalProductPackaged, MedicinalProductPharmaceutical, MedicinalProductUndesirableEffect, MessageDefinition, MessageHeader, MolecularSequence, NamingSystem, NutritionOrder, Observation, ObservationDefinition, OperationDefinition, OperationOutcome, Organization, OrganizationAffiliation, Parameters, Patient, PaymentNotice, PaymentReconciliation, Person, PlanDefinition, Practitioner, PractitionerRole, Procedure, Provenance, Questionnaire, QuestionnaireResponse, RelatedPerson, RequestGroup, ResearchDefinition, ResearchElementDefinition, ResearchStudy, ResearchSubject, RiskAssessment, RiskEvidenceSynthesis, Schedule, SearchParameter, ServiceRequest, Slot, Specimen, SpecimenDefinition, StructureDefinition, StructureMap, Subscription, Substance, SubstanceNucleicAcid, SubstancePolymer, SubstanceProtein, SubstanceReferenceInformation, SubstanceSourceMaterial, SubstanceSpecification, SupplyDelivery, SupplyRequest, Task, TerminologyCapabilities, TestReport, TestScript, ValueSet, VerificationResult, VisionPrescription } from './FhirType';
import { DocumentReferenceStatus_list_ValidValues } from '../FhirTsLibValidation/Fhir_ValidValues';
 ...
 *    file:/Users/admin/Documents/DevNetbeans/jsonSchemaToJava/target/classes/schema/fhir.schema.json#/definitions/DocumentReference
 */
export class DocumentReference {
  ...
  /**
   * A categorization for the type of document referenced - helps for indexing and searching. This may
   * be implied by or derived from the code specified in the DocumentReference.type.
   * Field access restricted because there are specific valid values.
   */
  @Type(() => CodeableConcept)
  ...
  protected relatesTo?: DocumentReferenceRelatesTo[];
   ...
   * DocumentReference.meta.security contains the security labels of the "reference" to the document,
   * while DocumentReference.securityLabel contains a snapshot of the security labels on the document
   * the reference refers to.
   * Field access restricted because there are specific valid values.
   */
  @Type(() => CodeableConcept)
  protected securityLabel?: CodeableConcept[];
  ...
  protected content?: DocumentReferenceContent[];
  ...
  */
  public setStatus(newValue: string) {
    ...
    const idx = getValidValueIdx(DocumentReferenceStatus_list_ValidValues, newValue);
    if (idx >= 0) {
      this.status = DocumentReferenceStatus_list_ValidValues[idx];
    } else {
      throw new Error("Unknown status of " + newValue);
    }
  }
  
  public getDocumentReferenceStatusValidValues(): string[] {
  ...
  * that should be caught.
  */
  public setDocStatus(newValue: string) {
    if (typeof(newValue) === 'undefined') {
      if (typeof(this.docStatus) !== 'undefined') {
         delete this.docStatus;
      }
      return;
    }
    
    if (REGEX_DOCSTATUS.test(newValue)) {
      this.docStatus = newValue
    } else {
      throw new Error("Invalid value pattern for docStatus of " + newValue);
    }
  }
  
  public getDocStatusPattern(): string {
  ...
  */
  public setDate(newValue: string) {
    ...
    if (REGEX_DATE_001.test(newValue)) {
      this.date = newValue
    } else {
  ...
  */
  ...
  public addSecurityLabel(newValue: CodeableConcept) {
    if (typeof(newValue) === 'undefined') {
