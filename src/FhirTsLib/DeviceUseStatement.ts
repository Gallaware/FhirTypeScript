 ...
 *    file:/Users/admin/Documents/DevNetbeans/jsonSchemaToJava/target/classes/schema/fhir.schema.json#/definitions/DeviceUseStatement
 */
export class DeviceUseStatement {
  ...
  /**
   * A code representing the patient or other source's judgment about the state of the device used that
   * this statement is about.  Generally this will be active or completed.
   * Field access restricted because there are specific valid values.
   */
  ...
  */
  public setStatus(newValue: string) {
    ...
    const idx = getValidValueIdx(DeviceUseStatementStatus_list_ValidValues, newValue);
    if (idx >= 0) {
      this.status = DeviceUseStatementStatus_list_ValidValues[idx];
    } else {
      throw new Error("Unknown status of " + newValue);
    }
  }
  
  public getDeviceUseStatementStatusValidValues(): string[] {
    return DeviceUseStatementStatus_list_ValidValues;
  }
  ...
  * that should be caught.
  */
  public setRecordedOn(newValue: string) {
    if (typeof(newValue) === 'undefined') {
      if (typeof(this.recordedOn) !== 'undefined') {
         delete this.recordedOn;
      }
      return;
    }
    
    if (REGEX_RECORDEDON.test(newValue)) {
      this.recordedOn = newValue
    } else {
      throw new Error("Invalid value pattern for recordedOn of " + newValue);
    }
  }
  
  public getRecordedOnPattern(): string {
    return REGEX_RECORDEDON.source;
  }
  
  /**
  * Gets the current value of field recordedOn.
  * @returns the value  or undefined
  */
  public getRecordedOn(): string | undefined {
      return this.recordedOn;
  }

  /**
  * Gets the current value of field recordedOn if it exists,
  * but if it is 'undefined' it will return the value supplied
  * as the parameter.
  * @returns the value or the parameter value
  */
  public getRecordedOnOrElse(defaultValue: string): string {
      if (this.recordedOn == undefined || this.recordedOn == null ) { return defaultValue; }
      return this.recordedOn;
  }
